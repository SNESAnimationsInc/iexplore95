...
#define ID_BUTTON_NEW_TAB 1010
#define ID_BUTTON_CLOSE_TAB 1011
...
#define DPIScale(x) ((int)((x) * SCALE_FACTOR))
...
#if 0
int DPIScale(HWND hParent, const TCHAR* scale) {
                ...
                env->CreateCoreWebView2Controller(hParent,
                    Callback<ICoreWebView2CreateCoreWebView2ControllerCompletedHandler>(
                        [hParent, tabIndex, url](HRESULT hr, ICoreWebView2Controller* controller) -> HRESULT {
                            ...
                            // --- NEW: DPI Scaling for WebView Only ---
                            // Get the DPI for the current window
                            UINT dpi = GetDpiForWindow(hParent);
                            double scale = dpi / 96.0;

                            // Apply the scale to the WebView rasterization
                            // 1. Create a pointer for the version 2 controller
                            Microsoft::WRL::ComPtr<ICoreWebView2Controller3> controller2;

                            // 2. Try to get the version 2 interface from your existing controller
                            if (SUCCEEDED(g_tabs[tabIndex].controller->QueryInterface(IID_PPV_ARGS(&controller2)))) {
                                // 3. Get the DPI scale (1.0 = 100%, 1.5 = 150%, etc.)
                                double scale = GetDpiForWindow(hParent) / 96.0;

                                // 4. Now you can call put_RasterizationScale
                                controller2->put_RasterizationScale(scale);
                            }
                            // ------------------------------------------
                            ...
                            // WebView starts below your Toolbar, Address Bar, and Tab Bar (90px total)
                            ...
                            g_tabs[tabIndex].webview->add_ContainsFullScreenElementChanged(
                                Callback<ICoreWebView2ContainsFullScreenElementChangedEventHandler>(
                                    [hParent](ICoreWebView2* sender, IUnknown* args) -> HRESULT {
                                        BOOL containsFull;
                                        sender->get_ContainsFullScreenElement(&containsFull);

                                        // Re-use your F11 logic to expand the window
                                        // If the video wants fullscreen, we toggle the window state
                                        ToggleFullscreen(hParent);

                                        return S_OK;
                                    }).Get(), nullptr);

                            g_tabs[tabIndex].webview->add_SourceChanged(
                                Callback<ICoreWebView2SourceChangedEventHandler>(
                                    [hParent](ICoreWebView2* sender, ICoreWebView2SourceChangedEventArgs* args) -> HRESULT {
                                        LPWSTR uri;
                                        sender->get_Source(&uri);
                                        if (uri) {
                                            // Update the Address Bar text
                                            HWND hAddress = GetDlgItem(hParent, ID_ADDRESS_BAR);
                                            SetWindowText(hAddress, uri);

                                            // Keep our global g_currentUrl in sync
                                            _tcsncpy_s(g_currentUrl, MAX_URL_LENGTH, uri, _TRUNCATE);
                            ...
                            g_tabs[tabIndex].webview->Navigate(TEXT("http://www.msn.com/?ocid=U220DHP&pc=U220"));
                            return S_OK;
                        }).Get());
                return S_OK;
            }).Get());
    ...
    UINT_PTR uIdSubclass, DWORD_PTR dwRefData)
{
    ...
    case WM_KEYDOWN:
        if (wParam == VK_F11) {
            ToggleFullscreen(GetParent(hwnd));
			SetFocus(hwnd);
            return 0;
        }
    ...
    UINT_PTR uIdSubclass, DWORD_PTR dwRefData)
{
    ...
    case WM_KEYDOWN:
        ...
        if (wParam == VK_RETURN) {
            ...
            HWND hAddress = GetDlgItem(hParent, ID_ADDRESS_BAR);
            SetWindowText(hAddress, finalUrl);

            if (g_currentTabIndex != -1 && g_tabs[g_currentTabIndex].webview) {
                g_tabs[g_currentTabIndex].webview->Navigate(finalUrl);
            }
            return 0;
        }
        break;
...
// Near the top of your file, add a global to store the menu
HMENU g_hMenuOriginal = NULL;

void ToggleFullscreen(HWND hwnd) {
    DWORD dwStyle = GetWindowLong(hwnd, GWL_STYLE);
    if (!g_isFullscreen) {
        // --- ADD THIS LINE: Initialize length before calling GetWindowPlacement ---
        g_wpPrev.length = sizeof(WINDOWPLACEMENT);

        if (GetWindowPlacement(hwnd, &g_wpPrev)) {
            MONITORINFO mi = { sizeof(mi), 0 };
            if (GetMonitorInfo(MonitorFromWindow(hwnd, MONITOR_DEFAULTTOPRIMARY), &mi)) {
                g_hMenuOriginal = GetMenu(hwnd);
                SetMenu(hwnd, NULL);

                ShowWindow(g_hToolbar, SW_HIDE);
                ShowWindow(g_hAddressBar, SW_HIDE);
                ShowWindow(g_hSearchBar, SW_HIDE);
                ShowWindow(g_hTabCtrl, SW_HIDE);
                ShowWindow(GetDlgItem(hwnd, ID_BUTTON_NEW_TAB), SW_HIDE);
                ShowWindow(GetDlgItem(hwnd, ID_BUTTON_CLOSE_TAB), SW_HIDE);
                ...
                if (g_currentTabIndex != -1 && g_tabs[g_currentTabIndex].controller) {
                    RECT fullRect = { 0, 0, mi.rcMonitor.right - mi.rcMonitor.left, mi.rcMonitor.bottom - mi.rcMonitor.top };
                    g_tabs[g_currentTabIndex].controller->put_Bounds(fullRect);
                }
            }
        }
        g_isFullscreen = true;
        ...
        if (g_hMenuOriginal) SetMenu(hwnd, g_hMenuOriginal);

        ShowWindow(g_hToolbar, SW_SHOW);
        ShowWindow(g_hAddressBar, SW_SHOW);
        ShowWindow(g_hSearchBar, SW_SHOW);
        ShowWindow(g_hTabCtrl, SW_SHOW);
        ShowWindow(GetDlgItem(hwnd, ID_BUTTON_NEW_TAB), SW_SHOW);
        ShowWindow(GetDlgItem(hwnd, ID_BUTTON_CLOSE_TAB), SW_SHOW);
        ...
        if (g_currentTabIndex != -1 && g_tabs[g_currentTabIndex].controller) {
            ...
            g_tabs[g_currentTabIndex].controller->NotifyParentWindowPositionChanged();
...
LRESULT CALLBACK NavigatorWindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_CREATE: {
        ...
        CreateWindowEx(
            ...
            0, 0, DPIScale(30), DPIScale(30), // We will position them in WM_SIZE
            hwnd, (HMENU)ID_BUTTON_NEW_TAB, g_hInstance, NULL
        );

        CreateWindowEx(
            0, TEXT("BUTTON"), TEXT("x"),
            WS_VISIBLE | WS_CHILD | BS_PUSHBUTTON,
            0, 0, DPIScale(30), DPIScale(30),
            hwnd, (HMENU)ID_BUTTON_CLOSE_TAB, g_hInstance, NULL
        );
    ...
    case WM_SIZE: {
        ...
        int addressH = 30;
        int tabH = 30;
        int buttonsWidth = DPIScale(60);

        // 1. Toolbar and Address Bar as before
        int searchBarWidth = DPIScale(350);
        MoveWindow(g_hToolbar, 0, 0, width, toolbarH, TRUE);
        MoveWindow(g_hAddressBar, 0, toolbarH, width - searchBarWidth, addressH, TRUE);
        MoveWindow(g_hSearchBar, width - searchBarWidth, toolbarH, searchBarWidth, addressH, TRUE);

        // 2. Tab Control - Make it slightly shorter to fit buttons on the right
        // We leave space for two buttons (60px scaled)
        MoveWindow(g_hTabCtrl, 0, toolbarH + addressH, width - buttonsWidth, tabH, TRUE);

        // 3. Position New Tab and Close Tab buttons next to the tabs
        HWND hBtnNew = GetDlgItem(hwnd, ID_BUTTON_NEW_TAB);
        HWND hBtnClose = GetDlgItem(hwnd, ID_BUTTON_CLOSE_TAB);

        // Y position is the same as the Tab Control (toolbarH + addressH)
        MoveWindow(hBtnNew, width - buttonsWidth, toolbarH + addressH, DPIScale(30), tabH, TRUE);
        MoveWindow(hBtnClose, width - (buttonsWidth / 2), toolbarH + addressH, DPIScale(30), tabH, TRUE);

        // 4. WebView fills the rest
        if (g_currentTabIndex != -1 && g_tabs[g_currentTabIndex].controller) {
            RECT bounds = { 0, toolbarH + addressH + tabH, width, height };
    ...
    case WM_COMMAND: {
        ...
        case ID_BUTTON_NEW_TAB:
            // Call your existing function with the home page
            CreateNewTab(hwnd, TEXT("http://www.msn.com/?ocid=U220DHP&pc=U220"));
            break;

        case ID_BUTTON_CLOSE_TAB: {
            if (g_tabs.size() > 1) {
                CloseTab(g_currentTabIndex, hwnd);
            }
            break;
        }
...
LRESULT CALLBACK PagSetWindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_PAINT: {
        ...
        HFONT hFont = CreateFont(DPIScale(-14), 0, 0, 0, FW_NORMAL, FALSE, FALSE, FALSE,
            ANSI_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS,
            DEFAULT_QUALITY, DEFAULT_PITCH | FF_SWISS, TEXT("Tahoma"));

        HFONT hSmallFont = CreateFont(DPIScale(-14), 0, 0, 0, FW_NORMAL, FALSE, FALSE, FALSE,
            ANSI_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS,
            DEFAULT_QUALITY, DEFAULT_PITCH | FF_SWISS, TEXT("Tahoma"));
...
LRESULT CALLBACK ChangelogWindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_CREATE:
        g_openWindowCount++;
        CreateWindowEx(
            0, TEXT("STATIC"), TEXT("- 0.7 beta: Fixed blurry effect; added New/Close tab buttons; added fullscreen support; fixed program running in background bug."),
            WS_VISIBLE | WS_CHILD,
            10, 75, 1024, 24,
            hwnd, (HMENU)6001, g_hInstance, NULL
        );
        
        CreateWindowEx(
            0, TEXT("STATIC"), TEXT("- 0.65 beta: The main window can now be closed without exiting the app; added tabs."),
            WS_VISIBLE | WS_CHILD,
            10, 110, 800, 24,
            hwnd, (HMENU)6002, g_hInstance, NULL
        );

        CreateWindowEx(
            0, TEXT("STATIC"), TEXT("- 0.6 beta: Added the menu bar (still in development); the browser can now open files; some bug fixes."),
            WS_VISIBLE | WS_CHILD,
            10, 145, 800, 24,
            hwnd, (HMENU)6003, g_hInstance, NULL
        );

        CreateWindowEx(
            0, TEXT("STATIC"), TEXT("- 0.5 beta: Added the basics (Search Bar, address bar, basic web navigation, etc.)"),
            WS_VISIBLE | WS_CHILD,
            10, 180, 800, 24,
            hwnd, (HMENU)6004, g_hInstance, NULL
        );
    ...
    case WM_PAINT: {
        ...
        const TCHAR* titleText1 = TEXT("Internet Explorer Version 0.7 beta");
...
int WINAPI wWinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrevInstance, _In_ LPWSTR lpCmdLine, _In_ int nCmdShow) {
    ...
    HWND hwnd = CreateWindowEx(
        ...
        CW_USEDEFAULT, CW_USEDEFAULT, DPIScale(800), DPIScale(600),
    ...
    CreateWindowEx(
        ...
        WS_TABSTOP | WS_VISIBLE | WS_CHILD | BS_DEFPUSHBUTTON,
        10, 75, DPIScale(125), DPIScale(50),
    ...
    CreateWindowEx(
        ...
        WS_TABSTOP | WS_VISIBLE | WS_CHILD | BS_DEFPUSHBUTTON,
        DPIScale(145), 75, DPIScale(150), DPIScale(50),
    ...
    CreateWindowEx(
        ...
        WS_TABSTOP | WS_VISIBLE | WS_CHILD | BS_DEFPUSHBUTTON,
        DPIScale(305), 75, DPIScale(150), DPIScale(50),
    ...
    CreateWindowEx(
        ...
        WS_TABSTOP | WS_VISIBLE | WS_CHILD | BS_DEFPUSHBUTTON,
        DPIScale(465), 75, DPIScale(125), DPIScale(50),
